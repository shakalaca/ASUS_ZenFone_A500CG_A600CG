...
dhd_get_suspend_bcn_li_dtim(dhd_pub_t *dhd)
{
	...
	/* if assoc grab ap's bcnprd value */
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_BCNPRD,
		&bcnprd, sizeof(bcnprd), FALSE, 0)) < 0) {
		DHD_ERROR(("%s failed code %d\n", __FUNCTION__, ret));
		goto exit;
	}

	DHD_ERROR(("%s bcn_li_dtim=%d DTIM=%d Listen=%d bcnprd=%d\n",
		__FUNCTION__, bcn_li_dtim, dtim_assoc, LISTEN_INTERVAL, bcnprd));
	...
	*  or if the becon peridod is too long.
	*/
	if ((dtim_assoc > CUSTOM_LISTEN_INTERVAL) || ((bcnprd * dtim_assoc) >= MAX_BCNPRD)) {
