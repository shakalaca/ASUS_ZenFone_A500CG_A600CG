			...
			if (asd->params.flash_state ==
				...
				if (frame->flash_state
					...
					dev_dbg(isp->dev,
						 "%s partially flashed\n",
						 __func__);
					printk("[FLASH] [Debug] ATOMISP_FRAME_STATUS_FLASH_PARTIAL ,sof id = %d \n",atomic_read(&asd->sof_count));
				} else if (frame->flash_state
					   == CSS_FRAME_FLASH_STATE_FULL) {
					asd->frame_status[vb->i] =
					...
					dev_dbg(isp->dev,
						 ...
						 __func__);
					printk("[FLASH] [Debug] ATOMISP_FRAME_STATUS_FLASH_EXPOSED ,sof id = %d \n",atomic_read(&asd->sof_count));
				} else {
					asd->frame_status[vb->i] =
					...
					dev_dbg(isp->dev,
						 ...
						 __func__);
					printk("[FLASH] [Debug] no flash in this frame ,sof id = %d \n",atomic_read(&asd->sof_count));
				}
				...
				if (asd->frame_status[vb->i] ==
					...
					asd->params.flash_state =
						ATOMISP_FLASH_DONE;
					printk("[FLASH] [Debug] ATOMISP_FLASH_DONE ,sof id = %d \n",atomic_read(&asd->sof_count));
				}
