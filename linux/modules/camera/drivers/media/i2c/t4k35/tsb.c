	...
	u16 gain, u16 digitgain)
{
	...
	gain = clamp_t(u16, gain, 40, TSB_MAX_GLOBAL_GAIN_SUPPORTED);
				...
				const struct tsb_reg *reglist)
{
	...
	ret = tsb_read_reg(client, 2, TSB_VERTICAL_START_H, data);
	...
	ret = tsb_read_reg(client, 2, TSB_VERTICAL_END_H, data);
...
#if 1	// TSB
	// temp. fixed to full-size (no-crop) for K37,K35
	//buf->crop_horizontal_end = data[0]-1;
