				...
				const struct imx_reg *reglist)
{
	...
	ret = imx_read_reg(client, 1, IMX_PLL_MULTIPLIER, data);
	if (ret)
		return ret;
	pll_multiplier = data[0];
	ret = imx_read_reg(client, 1, IMX_RGPLTD, data);
	if (ret)
		return ret;
	rpgltd = data[0] & IMX_MASK_2BIT;
	ret = imx_read_reg(client, 1, IMX_CCP2_DATA_FORMAT, data);
	if (ret)
		return ret;
...
static int get_resolution_index(struct v4l2_subdev *sd, int w, int h)
{
	...
	for (i = 0; i < dev->entries_curr_table; i++) {
		...
		/* Found it */
	        ...
	        if((binning_sum == 1) && (i + 1 < dev->entries_curr_table)){
			i++;
			printk("Select binning sum mode idx = %d\n",i);
	        }
		return i;
				...
				struct v4l2_mbus_framefmt *fmt)
{
	...
	if ((fmt->width > IMX111_RES_WIDTH_MAX)
		|| (fmt->height > IMX111_RES_HEIGHT_MAX)) {
		fmt->width =  IMX111_RES_WIDTH_MAX;
		fmt->height = IMX111_RES_HEIGHT_MAX;
	} else {
...
out:
	...
	exp_time = dev->coarse_itg * dev->pixels_per_line;
        if (exp_time >= (dev->vt_pix_clk_freq_mhz >> 4) &&
...
static int imx_detect(struct i2c_client *client, u16 *id, u8 *revision)
{
	...
	if (imx_read_reg(client, IMX_16BIT, IMX111_CHIP_ID, id)) {
...
static void imx_shutdown(struct i2c_client *client)
{
	imx_remove(client);
}
...
{
	dev->mode_tables = &imx111_sets;
	dev->vcm_driver = &imx111_vcms;
	dev->otp_driver = &imx111_otps;
