...
#if 0
static int gc2155_t_vflip(struct v4l2_subdev *sd, int value);
static int gc2155_t_hflip(struct v4l2_subdev *sd, int value);
#endif
...
static int gc2155_res2size(unsigned int res, int *h_size, int *v_size)
{
	...
	switch (res) {
	case GC2155_RES_QCIF:
		hsize = GC2155_RES_QCIF_SIZE_H;
		vsize = GC2155_RES_QCIF_SIZE_V;
		break;
#if 0
	case GC2155_RES_QVGA:
		hsize = GC2155_RES_QVGA_SIZE_H;
		vsize = GC2155_RES_QVGA_SIZE_V;
		break;
#endif
	case GC2155_RES_CIF:
	...
	case GC2155_RES_VGA:
		...
		break;
	case GC2155_RES_1600x900:
		hsize = GC2155_RES_1600x900_SIZE_H;
		vsize = GC2155_RES_1600x900_SIZE_V;
		break;
	case GC2155_RES_2M:
				  ...
				  struct v4l2_mbus_framefmt *fmt)
{
	...
	case GC2155_RES_QCIF:
		pr_info("gc2155_set_mbus_fmt RES: QCIF\n");
		ret = gc2155_write_reg_array(c, gc2155_qcif_init);
		if (ret) {
			pr_info("%s: set QCIF seq FAIL.\n", __func__);
			return -EINVAL;
		}
		break;
	case GC2155_RES_CIF:
		pr_info("gc2155_set_mbus_fmt RES: CIF\n");
		...
		break;
	case GC2155_RES_VGA:
		pr_info("gc2155_set_mbus_fmt RES: VGA\n");
		...
		break;
	case GC2155_RES_1600x900:
		pr_info("gc2155_set_mbus_fmt RES: 1600x900\n");
		ret = gc2155_write_reg_array(c, gc2155_1616x916_init);
		if (ret) {
			pr_info("%s: set 1600x900 seq FAIL.\n", __func__);
			return -EINVAL;
		}
		break;
	case GC2155_RES_2M:
		pr_info("gc2155_set_mbus_fmt RES: 2M\n");
...
#if 0
static int gc2155_g_2a_status(struct v4l2_subdev *sd, s32 *val)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	...
	*val = 0;
	...
	ret = gc2155_read_reg(client, GC2155_16BIT,
			GC2155_AE_TRACK_STATUS, &status_exp);
	if (ret)
		return ret;

	ret = gc2155_read_reg(client, GC2155_16BIT,
			GC2155_AWB_STATUS, &status_wb);
	if (ret)
		return ret;

	if (status_exp & GC2155_AE_READY)
		*val = V4L2_2A_STATUS_AE_READY;

	if (status_wb & GC2155_AWB_STEADY)
        ...
        struct atomisp_exposure *exposure)
{
    ...
    pr_info("%s(%d):Bon lsc = %d\n", __func__, __LINE__, lsc);
    if (ret) {
...
static int gc2155_t_hflip(struct v4l2_subdev *sd, int value)
{
	...
	int err;
	/* set for direct mode */
	err = gc2155_write_reg(c, GC2155_16BIT, 0x098E, 0xC850);
	if (value) {
		/* enable H flip ctx A */
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC850, 0x01, 0x01);
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC851, 0x01, 0x01);
		/* ctx B */
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC888, 0x01, 0x01);
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC889, 0x01, 0x01);

		err += gc2155_rmw_reg(c, GC2155_16BIT, GC2155_READ_MODE,
					GC2155_HFLIP_MASK, GC2155_FLIP_EN);

		dev->bpat = GC2155_BPAT_GRGRBGBG;
	} else {
		/* disable H flip ctx A */
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC850, 0x01, 0x00);
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC851, 0x01, 0x00);
		/* ctx B */
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC888, 0x01, 0x00);
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC889, 0x01, 0x00);

		err += gc2155_rmw_reg(c, GC2155_16BIT, GC2155_READ_MODE,
					GC2155_HFLIP_MASK, GC2155_FLIP_DIS);

		dev->bpat = GC2155_BPAT_BGBGGRGR;
	}

	err += gc2155_write_reg(c, GC2155_8BIT, 0x8404, 0x06);
	udelay(10);
...
static int gc2155_t_vflip(struct v4l2_subdev *sd, int value)
{
	...
	if (value >= 1) {
		/* enable H flip - ctx A */
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC850, 0x02, 0x01);
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC851, 0x02, 0x01);
		/* ctx B */
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC888, 0x02, 0x01);
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC889, 0x02, 0x01);

		err += gc2155_rmw_reg(c, GC2155_16BIT, GC2155_READ_MODE,
					GC2155_VFLIP_MASK, GC2155_FLIP_EN);
	} else {
		/* disable H flip - ctx A */
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC850, 0x02, 0x00);
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC851, 0x02, 0x00);
		/* ctx B */
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC888, 0x02, 0x00);
		err += gc2155_rmw_reg(c, GC2155_8BIT, 0xC889, 0x02, 0x00);

		err += gc2155_rmw_reg(c, GC2155_16BIT, GC2155_READ_MODE,
					GC2155_VFLIP_MASK, GC2155_FLIP_DIS);
	}
