...
static int hm2056_raw_try_res(struct v4l2_subdev *sd, u32 *w, u32 *h)
{
    ...
    for (ratio_correct = 0, idx_ratio = idx ; idx_ratio < dev->n_res ; idx_ratio++){
        sup_ratio = (dev->hm2056_raw_res[idx_ratio].width * 10)/(dev->hm2056_raw_res[idx_ratio].height);
        if((set_ratio > 15 && sup_ratio > 15) || (set_ratio < 15 && sup_ratio < 15)){
            ratio_correct = 1;
            break;
        }
    }
...
{
	struct hm2056_raw_device *dev = to_hm2056_raw_sensor(sd);
	*val = dev->hm2056_raw_res[dev->fmt_idx].bin_factor_x;
	return 0;
}
...
{
	struct hm2056_raw_device *dev = to_hm2056_raw_sensor(sd);
	*val = dev->hm2056_raw_res[dev->fmt_idx].bin_factor_y;
	return 0;
}
...
static void hm2056_raw_shutdown(struct i2c_client *client)
{
