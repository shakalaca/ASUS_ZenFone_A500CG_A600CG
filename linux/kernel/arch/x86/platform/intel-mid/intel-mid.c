...
static int __init intel_mid_platform_init(void)
{
	...
	default:
		...
		handle_sfi_table(SFI_SIG_GPIO, NULL, NULL, sfi_parse_gpio);
		handle_sfi_table(SFI_SIG_DEVS, NULL, NULL, sfi_parse_devs);
		handle_sfi_table(SFI_SIG_OEMR, NULL, NULL, sfi_parse_oemr);
		Read_HW_ID();
		Read_PROJ_ID();
		Read_PCB_ID();
		Read_TP_ID();
...
static int sim_id_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
        return count;
}
static int sim_id_proc_read(char *page, char **start, off_t off, int count, int *eof, void *date)
{
        int len=0;

        len=sprintf(page,
                        "%d\n",
                        (gpio_get_value(sim_id_gpionumber) ? 1 : 0)
                   );
        return len;
}
