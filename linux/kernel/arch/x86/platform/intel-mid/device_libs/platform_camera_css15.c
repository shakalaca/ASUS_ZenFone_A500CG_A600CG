...
const struct intel_v4l2_subdev_id v4l2_ids[] = {
	...
	{"hm2056", RAW_CAMERA, ATOMISP_CAMERA_PORT_SECONDARY},		// front camera
	{"gc2155", RAW_CAMERA, ATOMISP_CAMERA_PORT_SECONDARY},
// leong - Flash IC devices
	{"rt8515", LED_FLASH, -1},
	...
	if (!INTEL_MID_BOARD(1, TABLET, BYT) && camera_af_power_gpio == -1) {
		...
		if (gpio < 0) {
			pr_err("%s: can't find gpio `%s',default\n",
						__func__, gpio_name);
#ifdef A500CG	// leong  
			pr_err("%s: For A500CG , we don't use this CAM_0_AF_EN for power-up VCM ( ChungYi, leong ) \n",__func__);
#endif
			gpio = GPIO_DEFAULT;
		}
