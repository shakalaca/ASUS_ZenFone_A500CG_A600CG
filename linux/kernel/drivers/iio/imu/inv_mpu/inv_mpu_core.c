...
#define INV_RAW_DATA_VIA_I2C
#ifdef INV_RAW_DATA_VIA_I2C
...
{	
	...
	result = inv_i2c_read(st, REG_PWR_MGMT_2, 1,&data);
				printk("get_mpu65xx_gyro_rawdata power reg:%x (%x) +\n",REG_PWR_MGMT_2,data);
	buffer[6] = data;
        data&=(~BIT_PWR_GYRO_STBY);
...
{	
  ...
  struct inv_mpu_iio_s *st = iio_priv(dev_get_drvdata(dev));
	...
	short rawdata_x=0,rawdata_y=0,rawdata_z=0;
        ...
        result = inv_i2c_read(st, REG_PWR_MGMT_1, 1,&data);
        buffer[7]=data;
        data&=(~0x40);//disable sleep 
	//data=0;
       	result = inv_i2c_single_write(st, REG_PWR_MGMT_1, data);
	result = inv_i2c_read(st, REG_PWR_MGMT_2, 1, &data);
	printk("get_mpu65xx_gsensor_rawdata power (%x) +\n",data);
        buffer[6]=data;
        data&=(~BIT_PWR_ACCL_STBY);
	result = inv_i2c_single_write(st, REG_PWR_MGMT_2, data);
...
{	
  ...
  struct inv_mpu_iio_s *st = iio_priv(dev_get_drvdata(dev));
	...
	printk("secondary i2c addr:%x\n",st->plat_data.secondary_i2c_addr);
	result = inv_secondary_read(REG_AKM_ID, 1, &data);
	printk("get_mpu65xx_compassreg:%x_id: %x \n",REG_AKM_ID,data);
	...
	printk("compass rawdataX : (%d) | (%d)\n",buffer[0], buffer[1]);
	printk("compass rawdataY : (%d) | (%d)\n",buffer[2], buffer[3]);
	printk("compass rawdataZ : (%d) | (%d)\n",buffer[4], buffer[5]);
	//printk("compass rawdataX: %d\n",o[0]);
        //printk("compass rawdataY: %d\n",o[1]);
        //printk("compass rawdataZ: %d\n",o[2]);
...
#define INV_ACCL_CHAN(_type, _channel2, _index)                \
	{                                                         \
		...
		.modified = 1,                                        \
		.channel2 = _channel2,                                \
                .info_mask_separate = BIT(IIO_CHAN_INFO_CALIBBIAS)|BIT(IIO_CHAN_INFO_OFFSET),    \
                .info_mask_shared_by_type = BIT(IIO_CHAN_INFO_SCALE),   \
                .scan_index = _index,                                 \
		.scan_type  = IIO_ST('s', 16, 16, 0)                  \
	}
...
#ifdef INV_RAW_DATA_VIA_I2C
                                ARRAY_SIZE(inv_raw_i2c_mpu65xx_attributes)+
#endif
...
#ifdef INV_RAW_DATA_VIA_I2C// add for get raw data via I2C device attr
...
static void mpu6500_late_resume(struct early_suspend *h)
{
	printk("mpu6500_late_resume\n");
	if(enter_resume){
		printk("mpu6500_late_resume++\n");
		do_gettimeofday(&t2);
		int result;

		struct inv_mpu_iio_s *st = container_of(h, struct inv_mpu_iio_s, invmpu6500_early_suspend);
		...
		printk("mpu6500_late_resume elapsedtime --> %ld\n",elapsedTime);
		printk("mpu6500_late_resume sleeptime --> %ld\n",(SENSOR_UP_TIME-elapsedTime));

		if((SENSOR_UP_TIME-elapsedTime) > 0)
			msleep((SENSOR_UP_TIME-elapsedTime));
