...
//+ broadcom patch for improve sta/ap single channel performance
		if ((err = wldev_iovar_setint(_ndev, "ampdu_tx_lowat", 128)) != BCME_OK)
			WL_ERR(("failed to set ampdu_tx_lowat\n"));
//-
#ifdef PROP_TXSTATUS_VSDB
		...
		if (wl->p2p->vif_created) {
			...
			*/
			...
			if ((err = wldev_iovar_setint(dev, "bus:credall", 0)) != BCME_OK)
			{
				WL_ERR(("failed to set credall\n"));
//+ broadcom patch for improve sta/ap single channel performance
                                if ((err = wldev_iovar_setint(iter->ndev, "ampdu_tx_lowat", 1)) != BCME_OK)
                                        WL_ERR(("failed to set ampdu_tx_lowat\n"));
//-
			}
	...
	if (request) {		/* scan bss */
		...
		if (wl->iscan_on && (!ssids || !ssids->ssid_len || request->n_ssids != 1)) {
			...
			if (p2p_ssid) {
				if (wl->p2p_supported) {
					/* p2p scan trigger */
					wldev_iovar_setint(ndev, "mpc", 0);// HT PF400 remove again for scan hang
					if (p2p_on(wl) == false) {
...
wl_set_set_cipher(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if (sme->crypto.n_ciphers_pairwise) {
		...
		case WLAN_CIPHER_SUITE_SMS4:
			val = SMS4_ENABLED; //PF400 DONOT DEFINE
			pval = SMS4_ENABLED;
			break;
#endif
		default:
...
//patch for  reduce connection time when doing GO-NEG
#if 0
...
#else
        db_rssi = dtoh16(bi->RSSI) + RSSI_OFFSET;
        if((db_rssi <= -30) && (db_rssi > -35))
                db_rssi = db_rssi -3;
        else if((db_rssi <= -25) && (db_rssi > -30))
                db_rssi = db_rssi - 5;
...
//+ bcm patch for fix "Multi-channel: contineous fail to scan after authentication fail"
		/* Call wl_cfg80211_scan_abort() if assoc fail and driver status==CONNECTING*/
		wl_cfg80211_scan_abort(wl);
//-
...
//+ bcm patch for reduce GO negotiation fail rate
