...
//===================================================================
static ssize_t ps_calling_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < 10; i++) {
		length += sprintf(buf + length,
			...
			*(lp_info_cm36283->adc_table + i),
			i, *(lp_info_cm36283->cali_table + i),
...
static int initial_CM36283(struct CM36283_info *lpi)
{
	...
	ret_cm3628_or_cm36283 = I2C_RxData_to_judge_cm3628(0x61, &dummy, 1);
	D("[PS][CM36283] %s, ret is %d !!! \n", __func__, ret);
	if (!ret_cm3628_or_cm36283)  {
  		if (!record_init_fail )
  			record_init_fail = 1;
			
		D("[PS][CM36283] %s, is cm3628 old sensor \n", __func__);	
...
static int control_and_report( struct CM36283_info *lpi, uint8_t mode, uint16_t param ) {
  ...
  mutex_lock(&CM36283_control_mutex);
 D("[CM36283] control_and_report---mode = %d \n",mode);
 D("[CM36283] control_and_report---param = %d \n",param);
  if( mode == CONTROL_ALS ){
    if(param){
