...
#define DEF_PACKET_THRESH      DEF_GYRO_WAIT_TIME
...
static int inv_check_accl_self_test_raw(struct inv_mpu_iio_s *st,
    int *reg_avg, int accel_thres_mg)
{
    ...
    //accel_thres_mg = 500; //mg    
    accel_bias[X] = abs(reg_avg[X]*st->chip_info.multi);
    accel_bias[Y] = abs(reg_avg[Y]*st->chip_info.multi);
    accel_bias[Z] = abs(abs(reg_avg[Z]*st->chip_info.multi) - DEF_ST_PRECISION * DEF_ST_SCALE / (ACCL_G_SCALE << DEF_SELFTEST_ACCL_FULL_SCALE));
    thres = accel_thres_mg * DEF_ST_SCALE / (ACCL_G_SCALE << DEF_SELFTEST_ACCL_FULL_SCALE);
    printk("accel_bias=%d %d %d, thres=%ld(%d mg)\n", accel_bias[X], accel_bias[Y], accel_bias[Z], thres, accel_thres_mg);
    for (j = 0; j < 3; j++) {
        if (accel_bias[j] > thres) {
            ret_val |= (1<<j);
            printk("%s, axis(%d) > accel_thres_mg\n", __func__, j);
        }
    }
...
static int inv_check_gyro_self_test_raw(struct inv_mpu_iio_s *st,
    int *reg_avg, int gyro_thres_dps)
{
    int i, ret_val=0;
    long thres;

    //gyro_thres_dps = 20; //dps
#if 1
    thres = gyro_thres_dps*DEF_ST_PRECISION*DEF_ST_SCALE/(GYRO_DPS_SCALE << DEF_SELFTEST_GYRO_FULL_SCALE);
#else
    thres = 20*2*1000*131/4;
#endif
    printk("gyro_bias=%ld %ld %ld, thres=%ld(%d dps)\n", abs(reg_avg[X]), abs(reg_avg[Y]), abs(reg_avg[Z]), thres, gyro_thres_dps);
    for (i = 0; i < 3; i++) {
//        if (abs(reg_avg[i])*4 > 20*2*1000*131)
//            ret_val |= (1<<i);        
        if (abs(reg_avg[i]) > thres) {
            ret_val |= (1<<i);
            printk("%s, axis(%d) > gyro_thres_dps\n", __func__, i);
        }
	}
	...
	/* Self Test Pass/Fail Criteria C */
		...
		for (i = 0; i < 3; i++) {
			if(abs(reg_avg[i]) > gyro_offset_max) {
				printk("GYRO:CRITERIA C: Gyro axis:%d > 20dps\n", i);
				ret_val |= 1 << i;	//Error condition
			}
		}
	}
	...
	if(otp_value_zero == 0) {
		...
		for (i = 0; i < 3; i++) {
			...
			if(st_shift_cust[i] < accel_st_al_min || st_shift_cust[i] > accel_st_al_max) {
				printk("Accel axis:%d <= 225mg or >= 675mg\n", i);
				ret_val |= 1 << i;	//Error condition
			}
		}
	}
...
#if INV_DO_TEST_DEBUG
printk("sample_rate div=%d\n", test_setup.sample_rate);
printk("fifo_count=%d, packet_size=%d, packet_count=%d\n", fifo_count, packet_size, packet_count);
#endif
...
int inv_hw_self_test(struct inv_mpu_iio_s *st)
{
	...
	printk("Starting MPU Sensitity Self Test\n");
...
#define ACCEL_BIAS_THRESHOLD_SMT      500 //mg
#define GYRO_BIAS_THRESHOLD_SMT      20 //dps
...
//                        compass_result = !inv_check_compass_self_test(st);
       ...
       //accel_result = inv_check_accl_self_test(st, accl_bias_regular, accl_bias_st);
        ...
        accel_result |= inv_check_accl_self_test_raw(st, accl_bias_regular,ACCEL_BIAS_THRESHOLD_SMT);
        ...
        gyro_result |= inv_check_gyro_self_test_raw(st, gyro_bias_regular,GYRO_BIAS_THRESHOLD_SMT);
