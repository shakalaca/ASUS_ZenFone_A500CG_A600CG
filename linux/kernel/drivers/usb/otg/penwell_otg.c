...
static void penwell_otg_eye_diagram_optimize(void)
{
	...
	if (is_clovertrail(to_pci_dev(pnw->dev))) {
		...
		//20131203 ASUS-Eve_Wen change from 0x7f to 0x77
		//20131209 ASUS-Eve_Wen change from 0x77 to 0x7b
		value = eye_diagram_value;
		printk("penwell_otg_eye_diagram_optimize value = 0x%2x\n", value);
	} else {
...
//>>20131210ASUS-Eve_Wen
static ssize_t
tune_Eye_Diagram(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct penwell_otg              *pnw = the_transceiver;
	struct intel_mid_otg_xceiv      *iotg = &pnw->iotg;
	printk("tune_Eye_Diagram\n");
	if (count > 3)
	{		
		printk("tune_Eye_Diagram input value error! \n");
		return -1;
	}
	//printk("tune_eye_diagram buf = %x \n", buf);
	eye_diagram_value = simple_strtoul(buf, NULL, 16);
	penwell_otg_eye_diagram_optimize();
	printk("tune_eye_diagram  value = %x \n", eye_diagram_value);
	return count;
}//<<
static DEVICE_ATTR(eye_diagram, S_IRUGO | S_IWUSR | S_IWGRP, NULL, tune_Eye_Diagram);
		...
		const struct pci_device_id *id)
{
	...
	//>>20131210ASUS-Eve_Wen
	retval = device_create_file(&pdev->dev, &dev_attr_eye_diagram);
 	if (retval < 0) {
		dev_dbg(pnw->dev,
		"Can't eye_diagram sysfs attribute: %d\n", retval);
	 	goto err;
	}  //<<
